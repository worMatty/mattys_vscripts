...
function SpeakResponse(player, response) {
    if (response typeof == "integer") {
        response = responses[response];
    }

    player.AcceptInput("SpeakResponseConcept", response, null, null);
    ClientPrint(activator, Constants.EHudNotify.HUD_PRINTTALK, "Played response: " + response);
}

function SayNextResponse(startfrom = null) {
    if (startfrom != null) {
        response_count = startfrom;
    }
    if (response_count >= responses.len()) {
        response_count = 0;
        ClientPrint(activator, Constants.EHudNotify.HUD_PRINTTALK, "Reached end of list. Count reset to 0");
    }

    SpeakResponse(activator, response_count);
    response_count++;
}