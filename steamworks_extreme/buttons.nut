...
function Ready() {
	...
	EntFireByHandle(self, "Color", colours.ready, -1, activator, caller);
}
...
function Consume() {
	state.consumed = true;
	EntFireByHandle(self, "Color", colours.consumed, -1, activator, caller);
}
...
function Cooldown() {
	...
	EntFireByHandle(self, "Color", colours.cooldown, -1, activator, caller);
}
...
function Lock() {
	...
	if (state.consumed == false) {
		EntFireByHandle(self, "Color", colours.locked, -1, activator, caller);
	}
}
